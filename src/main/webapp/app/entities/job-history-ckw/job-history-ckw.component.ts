...
@Component({
    selector: 'jhi-job-history-ckw',
    templateUrl: './job-history-ckw.component.html'
})
export class JobHistoryCkwComponent implements OnInit, OnDestroy {
    currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.jobHistoryService
                ...
                .subscribe(
                    (res: HttpResponse<IJobHistoryCkw[]>) => this.paginateJobHistories(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/job-history-ckw'], {
            queryParams: {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: IJobHistoryCkw) {
        return item.id;
    }
    ...
    private paginateJobHistories(data: IJobHistoryCkw[], headers: HttpHeaders) {
